...
The gw↔ap-prov hop uses a dedicated wired backhaul over VLAN 99, currently via G.hn powerline adapters (GPL 2000S4). batman-adv runs both `phy1-mesh0` (WiFi) and the wired interface simultaneously; if the wired link drops batman falls back to WiFi automatically.
...
**This configuration is manual — not generated by `build.sh`.** If either device is reflashed, re-apply:

```bash
# gw: UCI VLAN device (run once, survives reboots)
...
```bash
# ap-prov: no UCI changes needed (eth1 is used directly, no VLAN sub-interface)
# Update /etc/init.d/batman-attach:
#!/bin/sh /etc/rc.common
START=99
start() {
    sleep 10
