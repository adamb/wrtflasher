...
**VLAN 99 — Wired mesh backhaul**: Dedicated point-to-point link between gw-office (port 1) and ap-prov (port 8). Carries batman-adv mesh traffic tagged. Port 8 also carries VLAN 10 untagged (PVID=10) for G.hn adapter management (192.168.1.241, 192.168.1.243). batman automatically prefers this link over WiFi due to lower latency and higher TQ score.
...
**Mesh backhaul (wired)**: VLAN 99 on eth0.99 (gw) / eth1.99 (ap-prov) — gw↔ap-prov only; both ends use tagged VLAN 99; switch port 8 also carries VLAN 10 untagged for G.hn adapter management; batman prefers this over WiFi automatically; WiFi remains active as fallback
...
The gw↔ap-prov hop uses a dedicated wired backhaul over VLAN 99, currently via G.hn powerline adapters (GPL 2000S4). batman-adv runs both `phy1-mesh0` (WiFi) and the wired interface (`eth0.99` on gw, `eth1.99` on ap-prov) simultaneously; batman automatically prefers the wired link due to lower latency and higher TQ score, and falls back to WiFi if the wired link drops.

**Switch config:** Port 1 (gw) carries VLAN 99 tagged. Port 8 (ap-prov/G.hn) carries VLAN 99 **tagged** and VLAN 10 **untagged** (PVID=10). Both ends use VLAN sub-interfaces (`eth0.99` on gw, `eth1.99` on ap-prov). VLAN 10 untagged on port 8 provides management access to the G.hn adapters (192.168.1.241, 192.168.1.243) on the LAN network.

**G.hn notes:** The GPL 2000S4 adapters pass 802.1Q tagged frames transparently. Powerline signal quality affects reliability; ensure both adapters are on the same circuit and no noise sources (UPS, surge protectors, large motors) are on the same circuit.

**G.hn FTP logging:** Both adapters (ghn-gw 192.168.1.241, ghn-prov 192.168.1.243) are configured to upload logs via FTP to deb (192.168.1.163) using the `nexus` login.

### Configuration Details

**AP-side configuration** (eth1.99 VLAN device, mesh_wired interface, batman-attach) **is generated by `build.sh`**. Gateway-side configuration (eth0.99) is manual — if the gateway is reflashed, re-apply the following settings.
