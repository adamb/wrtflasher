...
**Mesh backhaul (wired)**: VLAN 99 on eth0.99 (gw) / eth1.99 (ap-prov) — gw↔ap-prov only; batman prefers this over WiFi automatically; WiFi remains active as fallback
...
The gw↔ap-prov hop uses a dedicated wired backhaul over VLAN 99 via the Zyxel switch (port 1 ↔ port 8). batman-adv runs both `phy1-mesh0` (WiFi) and the VLAN interface simultaneously; if the cable is unplugged batman falls back to WiFi automatically.
...
```bash
# ap-prov: UCI VLAN device already in firmware config (network.vlan99 / network.vlan10)
# Just update /etc/init.d/batman-attach:
#!/bin/sh /etc/rc.common
START=99
start() {
    sleep 10
