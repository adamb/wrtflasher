...
| 8 | ap-prov (eth1) | 99 | Untagged |

**PVID Settings**: Port 4-5 = 10, Port 6-7 = 20, port 8 = 99, others = 1

**VLAN 99 — Wired mesh backhaul**: Dedicated point-to-point link between gw-office (port 1) and ap-prov (port 8). Carries only batman-adv OGMs and mesh traffic. No LAN/IoT/Guest traffic. batman automatically prefers this link over WiFi due to lower latency and higher TQ score.
...
**Mesh backhaul (wired)**: VLAN 99 on eth0.99 (gw) / eth1 untagged (ap-prov) — gw↔ap-prov only; switch port 8 strips the tag so G.hn adapters see plain Ethernet; batman prefers this over WiFi automatically; WiFi remains active as fallback
...
The gw↔ap-prov hop uses a dedicated wired backhaul over VLAN 99, currently via G.hn powerline adapters (GPL 2000S4). batman-adv runs both `phy1-mesh0` (WiFi) and the wired interface (`eth0.99` on gw, `eth1` on ap-prov) simultaneously; batman automatically prefers the wired link due to lower latency and higher TQ score, and falls back to WiFi if the wired link drops.

**Switch config:** Port 1 (gw) carries VLAN 99 tagged. Port 8 (ap-prov/G.hn) carries VLAN 99 **untagged** (PVID=99, untag egress member) — the switch strips the tag so the G.hn adapters see plain Ethernet frames. gw uses `eth0.99` (VLAN sub-interface); ap-prov uses `eth1` directly (no VLAN sub-interface needed since frames arrive untagged).

**G.hn notes:** The GPL 2000S4 adapters do not pass 802.1Q tagged frames — hence the untagged port 8 config. Powerline signal quality affects reliability; ensure both adapters are on the same circuit and no noise sources (UPS, surge protectors, large motors) are on the same circuit.

### Configuration Details

**This configuration is manual — not generated by `build.sh`.** If either device is reflashed, re-apply the following settings.
...
# No vlan10 device needed - eth1 is not used for VLAN 10
# (vlan10 should be removed if present)
...
start() {
    sleep 10
    batctl meshif bat0 if add phy1-mesh0 2>/dev/null || true
    batctl meshif bat0 if add eth1 2>/dev/null || true
}
```
...
1. Check that `eth1.10` is NOT in the `br-lan` bridge - this causes conflicts
2. Check that the `vlan10` device is removed - it creates eth1.10
3. Verify `eth1` is added to batman-adv via batman-attach script

**Activating after G.hn link is green (runtime, no reboot needed):**
```bash
# On ap-prov (if batman-attach didn't run yet)
ssh root@192.168.1.117 "batctl meshif bat0 if add eth1"
...
# Should show ap-prov's eth1 MAC (94:83:c4:7f:bb:ed) via eth0.99

ssh root@192.168.1.1 "batctl meshif bat0 originators | grep '94:83:c4:7f:bb:ec'"
# Best path (* line) should show eth0.99 as the interface with TQ near 255
```
